...
export default function SeedanceDetailsModal({ session, onClose, onReusePrompt }) {
  useEffect(() => {
  ...
  const formatModelLabel = (model) => {
    ...
    if (m.includes('i2v')) return 'Seedance 1.0 I2V';
    if (m.includes('t2v')) return 'Seedance 1.0 T2V';
    return 'Seedance 1.0';
  };
  ...
  const getDisplayStatus = () => {
    ...
    const isYellow = ['queued', 'queueing', 'running', 'processing'].includes(raw);
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="text-xs text-white/60">Details</div>
                 ...
                 <div className="flex items-center justify-between"><span className="text-white/60">Credits Used</span><span className="text-blue-300">{session.credit_cost || 5} credits</span></div>
